...
class JsonSafeLLMClient(OpenAIGenericClient):
    """LLM client with JSON keyword injection for Ollama/Groq compatibility.

    Groq and some OpenAI-compatible providers require the word 'json' in
    messages when response_format is used.  Graphiti's prompts don't always
    include it, causing 400 errors.  This wrapper injects a 'Respond in
    JSON format.' instruction when needed.

    Structured output (json_schema) is handled by the base class (v0.28.1+).
    """

    async def _generate_response(self, messages, response_model=None, **kwargs):
        # Inject 'json' keyword into messages if missing (Groq/Ollama requirement)
        has_json_mention = any(
            'json' in (m.content or '').lower()
            for m in messages
...
def create_graphiti(settings: ExtendedSettings) -> OpenClawGraphiti:
    ...
    if settings.reranker_provider == "openai":
        ...
        reranker = OpenAIRerankerClient(client=llm_client, config=reranker_config)
    else:
