...
const defaultConfig: EverMemOSConfig = {
  ...
  discoveryPollIntervalMs: 50, // Fast for tests
  discoveryTimeoutMs: 200,     // Short timeout for tests
};
...
describe("EverMemOSBackend", () => {
  ...
  describe("discoverFragmentIds", () => {
    test("discovers derived memory IDs via trace endpoint", async () => {
      const backend = new EverMemOSBackend(defaultConfig);

      // Store a memory to populate pendingStores
      mockFetch.mockResolvedValueOnce(jsonResponse({ count: 1 }));
      const result = await backend.store({ content: "Test", groupId: "eng" });
      const messageId = await result.fragmentId;

      // Mock trace endpoint: first poll returns "processing", second returns "complete"
      mockFetch
        ...
        .mockResolvedValueOnce(jsonResponse({
          message_id: messageId,
          status: "complete",
          memcell_ids: ["memcell-1"],
          derived_memories: {
            episodic_memory: ["obj-ep-1", "obj-ep-2"],
            foresight: ["obj-fs-1"],
            event_log: [],
          },
          all_ids: ["obj-ep-1", "obj-ep-2", "obj-fs-1"],
        }));

      const ids = await backend.discoverFragmentIds!(messageId);
      expect(ids).toEqual(["obj-ep-1", "obj-ep-2", "obj-fs-1"]);

      // Verify trace endpoint was called with the correct URL
      const traceCalls = mockFetch.mock.calls.filter(c =>
        (c[0] as string).includes("/api/v1/memories/trace/"),
      );
      expect(traceCalls.length).toBe(2);
      expect(traceCalls[0][0]).toContain(`/trace/${messageId}`);
    });

    test("returns empty on not_found", async () => {
      const backend = new EverMemOSBackend(defaultConfig);
    ...
    test("returns empty on timeout", async () => {
      const backend = new EverMemOSBackend({
        ...defaultConfig,
    ...
    test("returns empty for unknown messageId", async () => {
      const backend = new EverMemOSBackend(defaultConfig);

      const ids = await backend.discoverFragmentIds!("unknown-uuid");
      expect(ids).toEqual([]);
      // No fetch calls should be made
      expect(mockFetch).not.toHaveBeenCalled();
    });

    test("store populates pendingStores for discoverFragmentIds", async () => {
      const backend = new EverMemOSBackend(defaultConfig);

      mockFetch.mockResolvedValueOnce(jsonResponse({ count: 1 }));
      await backend.store({ content: "Test", groupId: "eng" });

      // discoverFragmentIds should find the pending store
      mockFetch.mockResolvedValueOnce(jsonResponse({
        status: "complete",
        all_ids: ["obj-1"],
      }));

      const ids = await backend.discoverFragmentIds!("test-uuid-000");
      expect(ids).toEqual(["obj-1"]);
    });
  });

  describe("resolveAnchors", () => {
    test("resolves anchor UUIDs to ObjectIds via trace endpoint", async () => {
      const backend = new EverMemOSBackend(defaultConfig);
      ...
      const resolved = await backend.resolveAnchors!(["anchor-a", "anchor-b"]);

      expect(resolved.size).toBe(2);
      expect(resolved.get("anchor-a")).toEqual(["obj-1", "obj-2"]);
      expect(resolved.get("anchor-b")).toEqual(["obj-3"]);
    });

    test("skips anchors that are not_found", async () => {
      const backend = new EverMemOSBackend(defaultConfig);
      ...
      const resolved = await backend.resolveAnchors!(["bad-id", "good-id"]);

      expect(resolved.size).toBe(1);
      expect(resolved.has("bad-id")).toBe(false);
      expect(resolved.get("good-id")).toEqual(["obj-1"]);
    });

    test("skips anchors that throw errors", async () => {
      const backend = new EverMemOSBackend(defaultConfig);
      ...
      const resolved = await backend.resolveAnchors!(["fail-id", "ok-id"]);

      expect(resolved.size).toBe(1);
      expect(resolved.get("ok-id")).toEqual(["obj-1"]);
    });

    test("returns empty map when all anchors fail", async () => {
      const backend = new EverMemOSBackend(defaultConfig);

      mockFetch.mockRejectedValue(new Error("All broken"));

      const resolved = await backend.resolveAnchors!(["a", "b"]);
